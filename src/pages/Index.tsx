        ...
        <div className="container">
          ...
          <div id="reviews-track" className="flex gap-5 overflow-x-auto scroll-smooth pb-4 snap-x snap-mandatory scrollbar-hide" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            {reviews.filter(r => r.visible).slice(0, 6).map((review, i) => {
              ...
              return (
                <motion.div key={review.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className={`min-w-[280px] max-w-[320px] flex-shrink-0 snap-start rounded-2xl border bg-card p-5 space-y-3 shadow-lg ${i === 2 ? "border-rose-500" : "border-rose-500"}`}>
